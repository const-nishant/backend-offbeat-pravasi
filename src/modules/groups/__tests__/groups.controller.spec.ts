...
describe('GroupsController', () => {
  ...
  it('PATCH /groups/:id — update', async () => {
    ...
    const result = await controller.update(mockUser as any, 'group-1', { name: 'Updated' });
    expect(result).toEqual(mockGroup);
  });

  it('POST /groups/:id/invite — invite', async () => {
    service.invite.mockResolvedValue([]);
    await controller.invite(mockUser as any, 'group-1', { invites: [{ email: 'a@b.com' }] });
    expect(service.invite).toHaveBeenCalledWith('group-1', 'user-1', expect.any(Object));
  });
  ...
  it('POST /groups/join/:shareCode — join', async () => {
    ...
    expect(service.join).toHaveBeenCalledWith('ABC123', 'user-1', 'user@test.com');
  });

  it('PATCH /groups/:id/members/:memberId/status — updateMemberStatus', async () => {
    service.updateMemberStatus.mockResolvedValue({} as any);
    await controller.updateMemberStatus(mockUser as any, 'group-1', 'member-1', {
      status: 'JOINED' as any,
    });
    expect(service.updateMemberStatus).toHaveBeenCalledWith('group-1', 'member-1', 'user-1', expect.any(Object));
  });

  it('DELETE /groups/:id/members/:memberId — removeMember', async () => {
    await controller.removeMember(mockUser as any, 'group-1', 'member-1');
    expect(service.removeMember).toHaveBeenCalledWith('group-1', 'member-1', 'user-1');
  });
