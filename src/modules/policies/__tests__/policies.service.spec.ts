...
describe('PoliciesService', () => {
  ...
  const mockPolicy = {
    ...
    isDefault: true,
    tiers: [
      { id: 'tier-1', policyId: 'policy-1', fromHoursBeforeStart: 168, toHoursBeforeStart: null, refundPercentage: 100, sortOrder: 1 },
      { id: 'tier-2', policyId: 'policy-1', fromHoursBeforeStart: 72, toHoursBeforeStart: 167, refundPercentage: 50, sortOrder: 2 },
      { id: 'tier-3', policyId: 'policy-1', fromHoursBeforeStart: 0, toHoursBeforeStart: 71, refundPercentage: 0, sortOrder: 3 },
    ],
  } as any;
  ...
  describe('findAll', () => {
    it('should return all policies ordered by createdAt desc', async () => {
      ...
      expect(policyRepo.find).toHaveBeenCalledWith({ order: { createdAt: 'DESC' } });
    });
  });
  ...
  describe('getDefault', () => {
    it('should return the default policy', async () => {
      ...
      expect(result).toEqual(mockPolicy);
      expect(policyRepo.findOne).toHaveBeenCalledWith({ where: { isDefault: true } });
    });
  ...
  describe('create', () => {
    const dto = {
      ...
      isDefault: true,
      tiers: [
        { fromHoursBeforeStart: 48, toHoursBeforeStart: undefined, refundPercentage: 100, sortOrder: 1 },
        { fromHoursBeforeStart: 0, toHoursBeforeStart: 47, refundPercentage: 0, sortOrder: 2 },
      ],
    };
    ...
    const savedPolicy = {
      ...
      isDefault: true,
      tiers: [
        { id: 'tier-1', policyId: 'new-policy', fromHoursBeforeStart: 48, refundPercentage: 100, sortOrder: 1 },
        { id: 'tier-2', policyId: 'new-policy', fromHoursBeforeStart: 0, toHoursBeforeStart: 47, refundPercentage: 0, sortOrder: 2 },
      ],
    } as any;

    it('should create a policy with tiers', async () => {
      policyRepo.findOne.mockResolvedValue(null);
      policyRepo.create.mockReturnValue({ id: 'new-policy', name: 'Flexible', description: 'Flexible policy', isDefault: true } as any);
      policyRepo.save.mockResolvedValue({ id: 'new-policy', name: 'Flexible', isDefault: true } as any);
  ...
  describe('getForTrek', () => {
    it('should return assigned policy when trek has explicit policy', async () => {
      trekPolicyRepo.findOne.mockResolvedValue({ policyId: 'policy-1', policy: mockPolicy } as any);
      policyRepo.findOne.mockResolvedValue(mockPolicy);
  ...
  describe('assignToTrek', () => {
    it('should create new link when none exists', async () => {
      policyRepo.findOne.mockResolvedValue(mockPolicy);
      trekPolicyRepo.findOne.mockResolvedValue(null);
      trekPolicyRepo.create.mockReturnValue({ trekId: 'trek-1', policyId: 'policy-1' } as any);
  ...
  describe('createSnapshot', () => {
    it('should create a snapshot from the current trek policy', async () => {
      ...
      expect(snapshotRepo.create).toHaveBeenCalledWith(
        expect.objectContaining({ bookingId: 'booking-1', policyName: 'Standard' }),
      );
  ...
  describe('calculateRefund', () => {
    ...
    it('should use snapshot tiers and return full refund when far out', async () => {
      ...
      const result = await service.calculateRefund('booking-1', 2000, futureDate);
