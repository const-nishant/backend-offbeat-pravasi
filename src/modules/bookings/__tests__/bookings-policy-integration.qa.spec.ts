...
class SqlitePayment {
  ...
  @Column({ type: 'varchar', length: 32, default: PaymentStatus.CREATED }) @Index()
...
describe('Bookings-Policy Integration QA (12y exp)', () => {
  ...
  beforeAll(async () => {
    dataSource = new DataSource({
      ...
      synchronize: true,
      entities: [
        SqliteUser, SqliteTrek, SqliteBooking, SqlitePayment,
        SqliteCancellationPolicy, SqliteCancellationTier, SqliteTrekPolicy, SqliteBookingPolicySnapshot,
      ],
    });
  ...
  function buildServices() {
    const settingsService = { getSettings: async () => ({ holdWindowMinutes: 15 }) } as any;
    ...
    const policyRepo = dataSource.getRepository(SqliteCancellationPolicy) as any;
    ...
    policiesService = new PoliciesService(
      policyRepo, tierRepo, trekPolicyRepo, snapshotRepo as any,
    );

    bookingsService = new BookingsService(
      bookingRepo as any, trekRepo as any, paymentRepo as any,
      settingsService, ticketService, dataSource,
      notificationsService, mailerService, policiesService,
    );
  }
  ...
  test('QA-2: Cancellation uses snapshot policy for refund calc, not current policy', async () => {
    ...
    const org = await userRepo.save(userRepo.create({
    ...
    const trek = await trekRepo.save(trekRepo.create({
    ...
    const booking = await bookingRepo.save(bookingRepo.create({
      ...
      trekSnapshot: { id: trek.id, name: trek.name, startDate: startDate.toISOString() },
    } as any));
    ...
    await snapshotRepo.save({
      ...
      tiers: [{ fromHours: 168, refundPercentage: 100, sortOrder: 1 },
  ...
  test('QA-3: Cancellation with succeeded payment stores refund metadata on payment record', async () => {
    ...
    const savedPayment = await paymentRepo.findOne({ where: { id: payment.id } as any });
  ...
  test('QA-7: Booking cancelled after trek already started gets 0 refund', async () => {
    ...
    const booking = await bookingRepo.save(bookingRepo.create({
      ...
      totalAmountInr: 5000,
      status: BookingStatus.CONFIRMED,
      trekSnapshot: { id: trek.id, name: trek.name, startDate: pastStart.toISOString() },
    } as any));
